...
instance Core.AWSService ListPermissionGroups where
...
instance Core.ToHeaders ListPermissionGroups where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "Content-Type"
...
instance Core.ToPath ListPermissionGroups where
  toPath = Prelude.const "/permission-group"

instance Core.ToQuery ListPermissionGroups where
