...
instance Core.ToHeaders GetRevealConfiguration where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "Content-Type"
...
instance Core.ToPath GetRevealConfiguration where
  toPath = Prelude.const "/reveal-configuration"

instance Core.ToQuery GetRevealConfiguration where
