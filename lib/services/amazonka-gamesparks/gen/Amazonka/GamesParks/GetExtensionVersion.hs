...
instance Core.AWSRequest GetExtensionVersion where
  ...
  response =
    Response.receiveJSON
      ( \s h x ->
          GetExtensionVersionResponse'
            Prelude.<$> (x Core..?> "ExtensionVersion")
            Prelude.<*> (Prelude.pure (Prelude.fromEnum s))
      )

instance Core.AWSService GetExtensionVersion where
...
instance Prelude.NFData GetExtensionVersion where
  rnf GetExtensionVersion' {..} =
    Prelude.rnf extensionVersion
      `Prelude.seq` Prelude.rnf name
      `Prelude.seq` Prelude.rnf namespace

instance Core.ToHeaders GetExtensionVersion where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "Content-Type"
...
instance Core.ToPath GetExtensionVersion where
  toPath GetExtensionVersion' {..} =
    Prelude.mconcat
      [ "/extension/",
        ...
        Core.toBS name,
        "/version/",
        Core.toBS extensionVersion
      ]

instance Core.ToQuery GetExtensionVersion where
