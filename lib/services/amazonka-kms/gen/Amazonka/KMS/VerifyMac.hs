...
verifyMac_message = Lens.lens (\VerifyMac' {message} -> message) (\s@VerifyMac' {} a -> s {message = a} :: VerifyMac) Prelude.. Core._Sensitive Prelude.. Core._Base64
...
verifyMac_mac = Lens.lens (\VerifyMac' {mac} -> mac) (\s@VerifyMac' {} a -> s {mac = a} :: VerifyMac) Prelude.. Core._Base64
...
instance Core.ToHeaders VerifyMac where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "X-Amz-Target"
              Core.=# ("TrentService.VerifyMac" :: Prelude.ByteString),
            "Content-Type"
...
instance Core.ToJSON VerifyMac where
  toJSON VerifyMac' {..} =
    Core.object
      ( Prelude.catMaybes
          [ ("GrantTokens" Core..=) Prelude.<$> grantTokens,
            ...
            Prelude.Just ("Mac" Core..= mac)
          ]
      )

instance Core.ToPath VerifyMac where
  toPath = Prelude.const "/"

instance Core.ToQuery VerifyMac where
