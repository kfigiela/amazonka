...
instance Core.ToHeaders CreateRuleset where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "Content-Type"
...
instance Core.ToJSON CreateRuleset where
...
instance Core.ToPath CreateRuleset where
  toPath = Prelude.const "/rulesets"

instance Core.ToQuery CreateRuleset where
